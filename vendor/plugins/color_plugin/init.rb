...
Raki::Plugin.register /^red|blue|green|yellow|grey|black|white|[0-9a-f]{3}|[0-9a-f]{6}$/i do
  ...
  execute do
    ...
    if callname.to_s =~ /^[0-9a-f]{3}|[0-9a-f]{6}$/i
      "<font style=\"color:##{callname.to_s};\">#{Raki.parser(context[:type]).parse body}</font>"
    elsif colors.key? callname.to_sym
      "<font style=\"color:##{colors[callname.to_sym]};\">#{Raki.parser(context[:type]).parse body}</font>"
    else
