...
different namespaces. It also supports a namespace-specific syntax.
...
<tt>config/raki.yml.example</tt> to <tt>config/raki.yml</tt> and edit it to fit your needs.
Run <tt>rake db:migrate</tt> to create all necessary tables.
...
to change the <tt>authenticator</tt> to <tt>openid</tt>. Then initialize and fetch the
grit submodule with <tt>git submodule init</tt> and <tt>git submodule update</tt>. The last step is to
